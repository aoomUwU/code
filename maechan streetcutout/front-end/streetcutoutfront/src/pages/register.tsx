...
const RegisterForm = (): JSX.Element => {
    ...
    const doRegister = async (e: { preventDefault: () => void; }) => {
        ...
        // ตรวจสอบว่าข้อมูล credential มีค่าไม่เป็นค่าว่าง
        if (!credential.email || !credential.first_name || !credential.last_name || !credential.password) {
            setError({ global: "กรุณากรอกข้อมูลให้ครบทุกช่องที่จำเป็น" });
            return;
        }
        ...
        if (validate()) {
            setIsLoading(true);
            try {
                let result = await call.post("maechan.maechan_streetcutout.api.test_register", {
                    'register': credential
                });
                // handle success, e.g., navigate to login
                console.log(result);
            } catch (error) {
                setError({ global: "เกิดข้อผิดพลาดในการลงทะเบียน โปรดลองอีกครั้ง" });
            } finally {
                setIsLoading(false);
            }
        }
